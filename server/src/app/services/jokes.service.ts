...
import { FilterJokeApi, Joke, NewJoke, UpdateJokeApi } from 'app/interfaces/joke.interfaces';
...
export class JokesService {
  ...
  public async sendRandomJoke(botToken: string, chatId: number): Promise<void> {
    ...
    const notSendedJokes = await this.jokesModel.getJokes({ sendedChatIds: {
      ids: [chatIdString],
      exclude: true,
    }});
    ...
    if (joke) {
      ...
      await this.telegramService.sendTextMessage(botToken, chatId, 'Jokes are over(((');
    }
  }

  private async sendJoke(botToken: string, chatId: number, joke: Joke): Promise<void> {
    const chatIdString = chatId.toString();
    const likes = await this.jokesLikesModel.getJokesLikes({ jokeId: joke.id });
    const rating = likes.reduce((sum, like) => sum + (like.isLike ? +1 : -1), 0) ?? 0;
    const likesCount = likes.reduce((sum, like) => sum + (like.isLike === true ? +1 : 0), 0) ?? 0;
    const dislikesCount = likes.reduce((sum, like) => sum + (like.isLike === false ? +1 : 0), 0) ?? 0;
    const markup = this.getJokeMarkup(rating, joke.id, likesCount, dislikesCount);
    await this.telegramService.sendMessageWithMarkup(botToken, chatId, joke.text, markup);
    await this.jokesModel.updateJoke({ id: joke.id, sendedChatIds: [...joke.sendedChatIds ?? [], chatIdString]}); // добавляются дублирующиеся id-шники чатов при поиске анеков
  }

  private getJokeMarkup(jokeRating: number, jokeId: number, likesCount: number, dislikesCount: number) {
    return {
      inline_keyboard: [
        [{
  ...
  public async likeJokeMessage(jokeId: number, userId: number, chatId: number, messageId: number, botToken: string, callbackQueryId: string): Promise<void> {
    const [ jokeLike ] = await this.jokesLikesModel.getJokesLikes({ jokeId, userId });
    ...
    await this.telegramService.answerCallbackQuery(botToken, callbackQueryId, callbackMessage);
  }

  public async dislikeJokeMessage(jokeId: number, userId: number, chatId: number, messageId: number, botToken: string, callbackQueryId: string): Promise<void> {
    const [ jokeLike ] = await this.jokesLikesModel.getJokesLikes({ jokeId, userId });
  ...
  public async updateJokeMessage(jokeId: number, chatId: number, messageId: number, botToken: string, callbackQueryId?: string): Promise<void> {
    try {
      ...
      if (joke) {
        ...
        const likesCount = likes.reduce((sum, like) => sum + (like.isLike === true ? +1 : 0), 0) ?? 0;
        const dislikesCount = likes.reduce((sum, like) => sum + (like.isLike === false ? +1 : 0), 0) ?? 0;
        const markup = this.getJokeMarkup(rating, jokeId, likesCount, dislikesCount);
        await this.telegramService.editMessageText(botToken, chatId, messageId, joke.text, markup);
      }
    } catch(err) {
      ...
      if (callbackQueryId) {
        await this.telegramService.answerCallbackQuery(botToken, callbackQueryId, 'Rating updated');
      }
    }
  }

  public async findJokeAndSend(botToken: string, chatId: number, text: string): Promise<void> {
    const jokes = await this.getJokes({ text });

    if (jokes && jokes[0]) {
      await this.sendJoke(botToken, chatId, jokes[0]);
    } else {
      await this.telegramService.sendTextMessage(botToken, chatId, 'Я не нашел ниче, по другому введи как-нибудь');
    }
  }

  public async sendJokesRating(botToken: string, chatId: number): Promise<void> {
    const query = 'select joke_id, sum(case when is_like then 1 else -1 end) as rating from jokes_likes group by joke_id order by rating desc';
    const { rows } = await this.postgres.customQuery<JokeIdWithRatingDb>(query);
    const ratingMessage = (await Promise.all(rows.map(async (row, index) => {
      const [ joke ] = await this.getJokes({ids: [row.joke_id]});

      if (joke) {
        return `${index + 1}. ${joke.text.substring(0, 40)}... (${row.rating})`;
      }
