...
export class ScheduleMessagesService {
  ...
  private scheduleMessage(rule: Rule, bot: Bot): void {
    ...
    const isJobAlreadyExist = this.jobs.has(jobId);
    ...
    if (
      ...
      rule.condition.scheduleChatIds.length
    ) {
          ...
          for (const chat of chats) {
            ...
            try {
              ...
              this.logger.errorLog(
                `Error then send schedule message: ${JSON.stringify(err)}`
              );
            }
          }
        } else {
          this.logger.errorLog(
            `Rule: ${rule.name} with not schedule condition type: ${rule.condition.type}`
          );
        }
      });
      ...
      this.logger.infoLog(
        `Job with id: ${jobId} scheduled on time: ${job.nextInvocation()}`
      );
    }
  }
  ...
  private getScheduleRule(rule: Rule): RecurrenceRule {
    ...
    if (
      rule.condition.type === "schedule" &&
      rule.condition.schedule.type === "weekly"
    ) {
      scheduleRule.dayOfWeek = this.getDayOfWeekIndex(
        rule.condition.schedule.dayOfWeek
      );
    ...
    if (
      rule.condition.type === "schedule" &&
      rule.condition.schedule.type === "annually"
    ) {
